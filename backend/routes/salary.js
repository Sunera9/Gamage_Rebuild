...
router.route("/search").get(async (req, res) => {
  ...
  try {
    ...
    // Handle userQuery for filtering by userId or userName
    if (userQuery) {
      let userMatch = null;

      // Check if userQuery matches a valid ObjectId (userId)
      if (mongoose.Types.ObjectId.isValid(userQuery)) {
        userMatch = await UserModel.findById(userQuery).select("_id");
      }

      // If not found by userId, search for userName
      if (!userMatch) {
        const usersByName = await UserModel.find({ name: userQuery }).select(
          "_id"
        );
    ...
    if (salaryComponents.length === 0) {
      return res
        .status(404)
        .json({ message: "No salary records found for the specified criteria." });
    }
...
router.route("/sheet").get(async (req, res) => {
  try {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      throw new Error(
        "Failed to construct valid date range for attendance query."
      );
    }
    ...
    const bulkOps = employees
      .map((employee) => {
        ...
        const standardAllowance = settingsMap["Standard Allowance"] || 0;
        const totalAllowances =
          medicalAllowance +
          dearnessAllowance +
          conveyanceAllowance +
        ...
        const epfEmployee =
          basicSalary * (settingsMap["EPF Employee"] / 100 || 0);
        const epfEmployer =
          basicSalary * (settingsMap["EPF Employer"] / 100 || 0);
        const etfEmployer =
          basicSalary * (settingsMap["ETF Employer"] / 100 || 0);
