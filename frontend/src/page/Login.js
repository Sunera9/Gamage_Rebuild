...
const Login = () => {
  ...
  const login = async (e) => {
    ...
    try {
      // API call to login
      const response = await axios.post("http://localhost:8070/auth/login", credentials, {
        headers: {
      ...
      Swal.fire({
        icon: "error",
        title: "Login Failed",
        text: error.response ? error.response.data.message : "Invalid credentials. Please try again.",
        confirmButtonText: "OK",
      });
    } finally {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-indigo-500 via-purple-600 to-pink-500">
      <div className="flex flex-col max-w-lg px-8 py-10 bg-white rounded-xl shadow-xl mt-10 mb-16 dark:bg-gray-800 sm:px-16 md:px-20 lg:px-24">
        ...
        <span className="justify-center text-sm text-center text-gray-500 flex-items-center dark:text-gray-400">
          ...
          <a href="/register" className="text-sm font-medium text-purple-600 underline hover:text-purple-800">
            Register now
          </a>
        </span>
        <div className="p-6 mt-6">
          <h1 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-4">Login</h1>
          <form onSubmit={login}>
            <div className="flex flex-col mb-4">
              <label className="text-sm font-medium text-gray-600 mb-1 dark:text-gray-400">Email Address</label>
            ...
            <div className="flex flex-col mb-4">
              <label className="text-sm font-medium text-gray-600 mb-1 dark:text-gray-400">Password</label>
              <div className="relative">
                <input
                  type="password"
                  name="password"
            ...
            <button
              type="submit"
              className={`py-3 px-6 bg-purple-600 hover:bg-purple-700 focus:ring-purple-500 focus:ring-offset-purple-200 text-white w-full transition ease-in duration-200 text-center text-lg font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={loading}
            >
